...
export type EventsListProps = {
  ...
  subtitle?: string;
  scope?: TimeScope;
  searchParams: { view?: string; page?: string; size?: string };
  /** Base path for the view-toggle links — defaults to /industry-events/list  */
  basePath?: string;
};
...
export async function EventsList({
  ...
  const now = new Date();
  const conditions: SQL[] = [eq(industryEvents.orgId, activeOrg.id)];
  if (scope === "upcoming") {
    conditions.push(gt(industryEvents.startAt, now));
  } else if (scope === "past") {
    conditions.push(lte(industryEvents.startAt, now));
  }

  const whereClause = and(...conditions);
  const { page, pageSize, offset } = parsePagination(searchParams);

  const [events, totalRow, categoryRows, organizerRows, locationRows] = await Promise.all([
    db
      .select()
      .from(industryEvents)
      .where(whereClause)
      .orderBy(
        scope === "upcoming"
          ? asc(industryEvents.startAt)
          : desc(industryEvents.startAt),
        desc(industryEvents.createdAt),
      )
      .limit(pageSize)
      .offset(offset),
    db.select({ c: count() }).from(industryEvents).where(whereClause),
    db
      .select()
  ...
  return (
    <div className="space-y-5">
          ...
          <div className="inline-flex items-center overflow-hidden rounded-[var(--r)] border border-border bg-card shadow-[var(--sh-xs)]">
            <Link
              ...
              className={`flex items-center gap-1 px-3 py-2 text-[12.5px] transition-colors ${
                isCalendarView ? "bg-pb-navy text-white" : "text-text-2 hover:bg-muted-bg"
              }`}
            ...
            <Link
              ...
              className={`flex items-center gap-1 px-3 py-2 text-[12.5px] transition-colors ${
                isListView ? "bg-pb-navy text-white" : "text-text-2 hover:bg-muted-bg"
              }`}
      ...
      {events.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-[var(--rlg)] border border-border bg-card px-6 py-20 shadow-[var(--sh-xs)]">
          ...
          <p className="mb-5 text-[12.5px] text-text-2">
            {scope
              ? "Events you create will show up here once dated."
      ...
      {/* Pagination — always shown so users can change rows-per-page even on empty states */}
      <div className="rounded-[var(--rlg)] border border-border bg-card shadow-[var(--sh-xs)]">
        <Pagination
          total={total}
          page={page}
          pageSize={pageSize}
          rowsOnPage={events.length}
        />
      </div>
    </div>
  );
